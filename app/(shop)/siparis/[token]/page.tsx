    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      {isNew && (
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="mt-3 text-2xl">Siparişiniz alındı!</h1>
          <p className="mt-2 text-sm text-zinc-600">
            Teşekkür ederiz. Sipariş detaylarınız aşağıdadır; bu sayfanın linkini saklayarak
      ...
      <div className="border border-line p-5">
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <span className="bg-black px-3 py-1 text-[13px] text-white">
        ...
        {order.status !== "cancelled" && (
          <ol className="mt-6 grid grid-cols-4 gap-1 text-center text-[11px] sm:text-xs">
            {STEPS.map((s, i) => (
              <li key={s}>
                <div className={cn("h-1 rounded-full", i <= stepIndex ? "bg-black" : "bg-line")} />
          ...
          <div className="mt-5 border border-black p-5 text-sm">
            ...
            {settings.iban ? (
              ...
              <p className="mt-2 text-zinc-600">
                Banka bilgileri için bizimle iletişime geçin: {settings.phone}
              </p>
            )}
            <p className="mt-3 text-[13px] text-zinc-600">
              Açıklama kısmına sipariş numaranızı (<b>#{order.orderNo}</b>) yazmayı unutmayın.
              Ödemeniz onaylandığında siparişiniz hazırlanmaya başlar.
            </p>
          </div>
        )}
          ...
          {order.items.map((item) => (
            <li key={item.id} className="flex gap-3 py-3">
              <div className="relative h-[90px] w-[60px] shrink-0 overflow-hidden bg-soft">
                {item.imageUrl && (
                  <Image src={item.imageUrl} alt="" fill sizes="60px" className="object-cover" />
                )}
              </div>
